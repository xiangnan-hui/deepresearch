...
"""
AI 资讯采集服务。招投标仅保留历史数据兼容接口，不再发起采集。
"""
import asyncio
import json
import logging
...
class NewsCollectionService:
    ...
    async def collect_news(self, max_items: int = 20, industry_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": bool(collected) or not errors,
    ...
    async def collect_bidding(self, *args, **kwargs):
        """已退出的业务；保留兼容返回，不再调用外部接口。"""
        return {"success": True, "collected": 0, "errors": [], "skipped": "招投标已停用"}

    async def collect_all(self, max_news=20, max_bidding=0, industry_id=None):
        config = get_industry_config(industry_id)
        result = await self.collect_news(max_news, config.id)
        return {"success": result.get("success", False), "news": result,
                "bidding": {"collected": 0, "skipped": "招投标已停用"}, "industry": config.name}
    ...
    def get_news_list(
        ...
        industry_id = get_industry_config(industry_id).id
    ...
    def has_data(self) -> bool:
        """检查是否有数据"""
        return self.get_news_stats()["total"] > 0
